...
## Task 1 - Alarm Clock 
Pintos Project 1의 첫 번째 과제는 기존에 구현된 바쁜 대기(Busy waiting) 방식 `timer_sleep()`을 개선하는 것을 목표로 한다. 초기 설계와 비교해 보았을 때 전체적인 설계는 같으며, 세부적 구현에서 '수면 중인' 스레드와 sleep 시간을 묶어서 저장하는 `alarm` 구조체에 들어가야 할 멤버를 구체화하였다는 점, 타이머 인터럽트 핸들러에서 알람 목록을 확인하고 스레드를 깨우는 과정이 리스트 순회로 더 구체적으로 구현되었다는 차이점이 있다.
...
Task 3는, design report의 설계와 비교해 보았을 때, 스레드 우선순위 설정을 위한 지표를 타이머 인터럽트 핸들러에서 업데이트한다는 아이디어를 더욱 구체화하여 커널이 다계층 피드백 큐 기반 스케줄러를 이용한다면 타이머 인터럽트 핸들러에서 `mlfqs_update()`를 호출하도록 변경하였으며, 스레드 우선순위 설정을 위한 지표를 계산하는 함수를 `update_load_avg()`, `calculate_recent_cpu()`, `calculate_priority()`로 구현하였다. 또한, MLFQS가 다음 실행할 스레드를 찾는 알고리즘을 `mlfqs_next_thread_to_run()` 함수로 구현하였다.
...
#### `fixed.h`
실수 계산을 위한 고정소수점 연산 루틴을 구현한 함수로, Pintos 문서에 나와있는것과 거의 비슷하다. `typedef`를 이용하여 고정소수점 기반 실수형을 `fixed` 키워드로 정의할 수 있도록 하였다. 또한, 이름 충돌 문제를 일으키는 이름을 조금 더 명확한 이름으로 변경하였다. 
