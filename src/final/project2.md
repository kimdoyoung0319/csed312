...
#### Who is responsible for destroying dead processes?
이번 과제에서는 프로세스가 종료되더라도, 종료된 프로세스를 나타내는 
`struct process`가 종료 상태를 전달하기 위해 메모리 상에서 할당 해제되지 않고
계속 남아있게 된다. 메모리 동적 할당시에는 할당된 메모리를 결국 해제할 때 그
해제 책임, 혹은 소멸 책임을 누가 져야 하는지가 중요하게 다루어진다. 이 문제를
해결하기 위해 프로세스가 종료되는 세 가지 경우를 생각해 보자. 

먼저 첫 번째 경우는 프로세스가 부모 프로세스가 존재하지 않는 고아 
프로세스이거나, 프로세스를 생성한 주체가 사용자 프로세스가 아닌 커널 스레드라서
이들이 할당 해제 책임을 질 수 없는 경우이다. `struct thread`에는 자식 프로세스의
목록을 저장하지 않기 때문에, 커널 스레드는 실행한 프로세스의 목록을 알 지 
못하며, 부모 프로세스가 이미 종료된 경우에도 자식 프로세스를 종료할 수 없다.
이 경우에는, 해당 프로세스 자기 자신이 메모리 할당 해제의 책임을 진다. 

두 번째 경우는 프로세스를 부모 프로세스가 `process_wait()`로 대기하고 있는
경우이다. 이 경우에는 부모 프로세스가 이 프로세스의 종료 상태를 받아야 하므로
종료하는 프로세스가 자기 자신을 소멸시키면 안된다. 따라서, 이 경우에는 부모 
프로세스가 자식 프로세스의 종료를 세마포어를 통해 전달받은 후 바로 해당 
프로세스에 대한 소멸 책임을 진다. 이때, 부모 프로세스는 이미 자식 프로세스가 
종료된 것을 알고, Pintos 문서의 `wait()` 시스템 호출의 의미(semantics)에 따르면 
한 프로세스를 두 번 이상 대기할 수 없다고 명시되어 있으므로 종료 상태를 전달받은
후 바로 해당 자녀 프로세스를 소멸시키는 것은 안전하다.

세 번째 경우는 프로세스가 고아 프로세스가 아니면서 부모 프로세스가 해당 
프로세스를 대기하고 있지도 않는 경우이다. *이 경우 또한 종료되는 프로세스는 자기
자신을 소멸시키면 안된다.* 왜냐하면, 부모 프로세스는 자식 프로세스가 종료된
이후에도 `wait()`를 호출해 해당 프로세스에 대기하고자 할 수 있으며, 이 경우에도
`wait()` 시스템 호출은 정상적으로 자식 프로세스의 종료 상태를 반환해야 하기 
때문이다.

이 경우의 소멸 책임은 누가 져야 할까? 이때 중요한 점은 부모 프로세스가 종료되면
그 부모 프로세스의 자식 프로세스에 `wait()`를 호출할 주체도 사라진다는 점이다.
따라서, 이 경우의 소멸 책임은 부모 프로세스가 지되, 부모 프로세스는 자기 자신이
종료될 때 종료 상태, 즉 `PROCESS_DEAD` 상태인 자식 프로세스들만을 소멸시켜야 
한다. 이후 부모 프로세스가 할당하지 못한 `PROCESS_ALIVE` 상태의 프로세스들은
자신이 고아가 되었음을 `struct process`의 `parent` 멤버를 통해 알 수 있으며,
이후 상술한 바와 같이 종료할 때 자기 자신에 대한 소멸 책임을 진다.
...
이번 과제를 통해, 운영체제에서 사용자 프로그램이 커널의 기능을 사용해야 할 때
호출하는 시스템 호출의 실제 구현 방법에 대해 알아볼 수 있었다. 또한 시스템 
호출을 구현할 때, 사용자가 전달하는 정보를 검증할 필요가 있음을 이해하고, 어떻게
사용자가 전달한 주소를 검증해야 하는 지를 알 수 있었다. 또한, 독립된 주소 공간을
가지는 프로그램의 실행 단위인 프로세스를 지원하기 위한 자료구조와 프로세스에
대한 인터페이스 함수, 특히 프로세스의 실행을 대기하거나 실행 파일로부터 새로운
프로세스를 시작하는 함수를 구현하며, 정적인 디스크 상의 프로그램에서 동적 객체인
프로세스를 만들어내는 과정과 이러한 프로세스가 제대로 동작하기 위한 운영체제
계층에서의 지원에는 어떤 것들이 필요한지를 알 수 있었다.

Pintos는 현재 하나의 프로세스에 하나의 스레드만을 지원하는 싱글 스레드 
운영체제이다. 하지만, 실제로는 대다수의 운영체제들이 하나의 프로세스에 여러
스레드를 생성할 수 있도록 지원하고 있다. 따라서, Pintos의 현재 구조를 수정하여
하나의 프로세스가 여러 스레드를 생성할 수 있도록 하고 이를 위한 시스템 호출을
사용자에게 지원한다면 더욱 실제 운영체제에 가까운 구현이 될 것이다.